...
export class VendorMasterComponent implements OnInit {
  ...
  displayedVendorLogListColumns: string[] = ['srno', 'invoice_no', 'receipt_no', 'receipt_date', 'generated_by', 'quantity'];
      ...
      if (result && result.length > 0) {
        ...
        const DATA = recordArray.reduce((current, next, index) => {
          ...
          next.pm_item_details.forEach(element => {
            quantity = Number(quantity) + Number(element.item_quantity);
          });
          current.push({
            "srno": index + 1,
            "invoice_no": next && next.pm_details && next.pm_details.invoice_no ? next.pm_details.invoice_no : '-',
