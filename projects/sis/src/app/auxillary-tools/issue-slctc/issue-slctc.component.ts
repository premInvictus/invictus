...
@Component({
	selector: 'app-issue-slctc',
	templateUrl: './issue-slctc.component.html',
	styleUrls: ['./issue-slctc.component.scss']
})
export class IssueSlctcComponent implements OnInit {
	tc_id;
	tcdetails: any = {};
	issueform: FormGroup;
	classArray = [];
	...
	saveForm() {
		if (this.issueform.valid) {
			this.issueform.value.tcd_doc_type = 'issued';
			this.sisService.issueSlcTc(this.issueform.value).subscribe((result: any) => {
				if (result && result.status === 'ok') {
