...
@Component({
  selector: 'app-followup-remarks-theme-two',
  templateUrl: './followup-remarks-theme-two.component.html',
  styleUrls: ['./followup-remarks-theme-two.component.css']
})
export class FollowupRemarksThemeTwoComponent implements OnInit, OnChanges {

  confirmValidParentMatcher = new ConfirmValidParentMatcher();
	@ViewChild('picker_parent') picker_parent;
	@ViewChild('picker') picker;
	followupRemarkForm: FormGroup;
	...
	buildForm() {
		this.followupRemarkForm = this.fbuild.group({
			era_type: 'followup',
			era_doj: new Date(),
			era_aut_id: '',
			era_ar_id: '',
    ...
    console.log('this.followupRemarkForm.value',this.followupRemarkForm.value);
		if (this.followupRemarkForm.valid) {
      this.followupRemarkForm.patchValue({
				'era_aut_id': 0,
			});
			this.finalGeneralRemarkArray.push(this.followupRemarkForm.value);
	...
	updateGeneralRemarkList() {
		this.finalGeneralRemarkArray[this.generalRemarkValue] = this.followupRemarkForm.value;
		this.prepareGroupAuthorityData(this.finalGeneralRemarkArray);
		this.notif.showSuccessErrorMessage('General Remark List Updated', 'success');
		this.followupRemarkForm.reset();
	...
	addEvent(type: string, event: MatDatepickerInputEvent<Date>) {
		...
		const convertedDate = datePipe.transform(this.followupRemarkForm.value.era_doj, 'yyyy-MM-dd');
		this.followupRemarkForm.patchValue({ 'era_doj': convertedDate });
	}
