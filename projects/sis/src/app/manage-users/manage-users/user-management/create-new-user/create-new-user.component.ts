...
export class CreateNewUserComponent implements OnInit {
	...
	addUser() {
		...
		if (!this.Cu_Form.value.au_full_name) {
			this.commonAPIService.showSuccessErrorMessage('FullName is Required', 'error');
		}
		if (!this.Cu_Form.value.au_password) {
			this.commonAPIService.showSuccessErrorMessage('Password is Required', 'error');
		}
		if (!this.Cu_Form.value.au_mobile) {
			this.commonAPIService.showSuccessErrorMessage('Mobile Number is Required', 'error');
		}
		...
		if (!this.Cu_Form.value.au_dob) {
			this.commonAPIService.showSuccessErrorMessage('Dob is Required', 'error');
		}
		if (!this.Cu_Form.value.au_role_id) {
			this.commonAPIService.showSuccessErrorMessage('Access Level is Required', 'error');
		}
      ...
      newUserFormData.append('au_role_id', '2'); */
			...
			if (this.Cu_Form.value.au_dob < today && this.Cu_Form.value.au_dob !== today && checkDateArray - birthDate > 14) {
				this.manageUsersService.addUser(this.Cu_Form.value).subscribe(
					(result: any) => {
						if (result && result.status === 'ok') {
							...
							if (result.data.url) {
								...
								xhr.open('GET', encodeURI(result.data.url), true);
								xhr.send();
							}
							this.saveief();
							this.commonAPIService.showSuccessErrorMessage('User Created Successfully', 'success');
							this.router.navigate(['../manage-access-user', this.userIdManageAccess], { relativeTo: this.route });
						}
					}
				);
			} else {
				this.commonAPIService.showSuccessErrorMessage('Please enter correct dob', 'error');
			}
		} else {
	...
	updateUser() {
      ...
      newUserFormData.append('au_role_id', '2'); */
			...
			const checkDateArray = Number(checkdate[0]);
			if (this.Cu_Form.value.au_dob < today && this.Cu_Form.value.au_dob !== today && checkDateArray - birthDate > 14) {
				this.manageUsersService.updateUser(this.Cu_Form.value).subscribe(
					(result: any) => {
						if (result && result.status === 'ok') {
							this.userIdManageAccess = result.data.login_id;
							const url = result.data.url;
							this.loading = false;
							this.commonAPIService.showSuccessErrorMessage('User Updated Successfully', 'success');
			...
			this.commonAPIService.showSuccessErrorMessage('User not Update', 'error');
		}
		this.loading = false;
	}
	...
	setUserForm(value) {
		this.Cu_Form.controls.au_login_id.setValue(value.au_login_id);
		...
		this.Cu_Form.controls.up_allow_sms.setValue(value.privileges[0] && value.privileges[0].up_allow_sms === '1' ? true : false);
		this.Cu_Form.controls.up_allow_email.setValue(value.privileges[0] && value.privileges[0].up_allow_email === '1' ? true : false);
		this.Cu_Form.controls.up_change_date.setValue(value.privileges[0] && value.privileges[0].up_change_date === '1' ? true : false);
		this.Cu_Form.controls.up_switch_tp.setValue(value.privileges[0] && value.privileges[0].up_switch_tp === '1' ? true : false);
		this.Cu_Form.controls.up_read_only.setValue(value.privileges[0] && value.privileges[0].up_read_only === '1' ? true : false);
	}
