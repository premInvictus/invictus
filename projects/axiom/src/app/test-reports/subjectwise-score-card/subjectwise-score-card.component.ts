...
@Component({
  selector: 'app-subjectwise-score-card',
  templateUrl: './subjectwise-score-card.component.html',
  styleUrls: ['./subjectwise-score-card.component.css']
})
export class SubjectwiseScoreCardComponent implements OnInit {

  @ViewChild(MatSort) sort: MatSort;
  reportArray: any[] = [];
  subject_reportArray: any[] = [];
  subjectArray: any[] = [];
	rankArray: any[] = [];
	login_id: string;
	es_id: number;
	...
	ngOnInit() {
		...
		this.testReportOverviewSubject();
  ...
  getSubjectScore(login_id,qus_sub_id) {
	...
	testReportOverviewSubject() {
		this.reportArray = [];
		this.reportService.testReportOverviewSubject({ es_id: this.es_id }).subscribe(
			(result: any) => {
				if (result && result.status === 'ok') {
          this.reportArray = result.data.overview_report;
          this.subject_reportArray = result.data.subject_report;
          if(this.subject_reportArray.length > 0){
            this.subject_reportArray.forEach(e => {
              const sind = this.subjectArray.findIndex(f => f.qus_sub_id == e.qus_sub_id);
              if(sind == -1){
                this.subjectArray.push({
                  qus_sub_id:e.qus_sub_id,
                  sub_name:e.sub_name
                })
              }
            })
          ...
          let ind = 1;
          this.reportdisplayedColumns =	['position', 'admissionNo', 'studentName', 'rank',
          'max_marks', 'attempted', 'correct', 'incorrect', 'skip'];
          this.subjectArray.forEach(e => {
            this.reportdisplayedColumns.push('sub'+e.qus_sub_id);
          });
          this.reportdisplayedColumns.push('score');
          this.reportdisplayedColumns.push('percentage');
          this.reportdisplayedColumns.push('cutoff');
          this.reportdisplayedColumns.push('status');
					for (const item of this.reportArray) {
						this.REPORT_ELEMENT_DATA.push({
							position: ind,
							admissionNo: item.au_login_id,
							loginid: item.eva_login_id,
