...
@Component({
  selector: 'app-detailed-greensheet',
  templateUrl: './detailed-greensheet.component.html',
  styleUrls: ['./detailed-greensheet.component.css']
})
export class DetailedGreenSheetComponent implements OnInit {
  ...
  getColorForStatus(marks){
    if(Number(marks) >= 33){
      return "rgb(194, 158, 159)"
    }else{
      return "#f8696b"
    }
  }

  getColorCode(percentage, max_marks){
    // let colors = [
    //   "#839872",
    //   "#8ea37d",
    //   "#9aae88",
    ...
    // console.log("per >>>>>>>>>>",typeof percentage, max_marks);
    
    let diff = Number(max_marks)/colors.length;
    // console.log(">>>>>i ", typeof(percentage) , ">>>>>> j", typeof(max_marks), ">>>> diff", diff);
    let k = 0;
    
    if(typeof percentage == "string" && percentage == "AB"){
      return "#FFFF00"
    }
    ...
    if(percentage == 0){
      ...
      for(let i = Number(max_marks); i > 0; i=i-diff){
        // for(let j = Number(max_marks)-diff; j > 0; j-diff){
          // console.log(">>>>>k ", k);
          if(percentage <= i && percentage > i-diff){
            return colors[k];
          }
          k++;
    ...
    //   return "#c8deb5";
    // }else if(percentage >= 20 && percentage < 33){
    //   return "#e1e8da";
