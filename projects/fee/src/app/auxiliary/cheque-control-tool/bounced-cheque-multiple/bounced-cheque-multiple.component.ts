...
export class BouncedChequeMultipleComponent implements OnInit {
  ...
  downloadPdf() {
	  ...
	  var toDate1 = this.bouncedForm.value.fcc_deposite_date ? this.bouncedForm.value.fcc_deposite_date: '';
		...
		setTimeout(() => {
          ...
          cellPadding : 5
			...
			doc.autoTable({
				head: [[
          {content: 'Date : '+(fromDate1 ? this.commonAPIService.dateConvertion(fromDate1, "dd-MMM-yyyy"):''),  styles: {halign: 'left', fillColor: '#ffffff'}}, 
      ...
      doc.autoTable({
				head: [[
          {content: '',  styles: {halign: 'left', fillColor: '#ffffff'}}, 
          {content: '',  styles: {halign: 'center', fillColor: '#ffffff'}},
					{content: 'Deposited By  ',  styles: {halign: 'right', fillColor: '#ffffff'}}]],
