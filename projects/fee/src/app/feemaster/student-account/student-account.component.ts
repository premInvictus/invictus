	...
	getFeeAccount(au_login_id) {
		...
		this.feeService.getFeeAccount({ accd_login_id: au_login_id }).subscribe((result: any) => {
			if (result && result.status === 'ok') {
				this.existFlag = true;
				accountDetails = result.data[0];
				if (accountDetails.accd_is_transport === 'Y') {
					this.transportFlag = true;
				} else {
					this.transportFlag = false;
				}
				if (accountDetails.accd_tr_id === '1') {
					this.modeFlag = true;
				} else {
					this.modeFlag = false;
				}
				if (accountDetails.accd_is_terminate === 'Y') {
				...
				if (accountDetails.accd_is_hostel_terminate === 'Y') {
				...
				if (accountDetails.accd_is_hostel === 'Y') {
					this.hostelFlag = true;
				} else {
					this.hostelFlag = false;
				}
				this.accountsForm.patchValue({
					accd_id: accountDetails.accd_id,
					accd_login_id: accountDetails.accd_login_id,
					accd_fo_id: accountDetails.accd_fo_id,
					accd_fs_id: accountDetails.accd_fs_id,
					accd_fcg_id: accountDetails.accd_fcg_id,
					accd_is_transport: accountDetails.accd_is_transport === 'N' ? false : true,
					accd_is_hostel: accountDetails.accd_is_hostel === 'N' ? false : true,
					accd_transport_mode: accountDetails.accd_transport_mode,
					accd_tr_id: accountDetails.accd_tr_id,
					accd_tsp_id: accountDetails.accd_tsp_id,
					accd_ts_id: accountDetails.accd_ts_id,
					accd_is_terminate: accountDetails.accd_is_terminate === 'N' ? false : true,
					accd_is_hostel_terminate: accountDetails.accd_is_hostel_terminate === 'N' ? false : true,
					accd_transport_from: accountDetails.accd_transport_from,
					accd_transport_to: accountDetails.accd_transport_to,
					accd_remark: accountDetails.accd_remark,
					accd_hostel_fs_id: accountDetails.accd_hostel_fs_id,
					accd_hostel_fcc_id: accountDetails.accd_hostel_fcc_id,
					accd_hostel_from: accountDetails.accd_hostel_from,
					accd_hostel_to: accountDetails.accd_hostel_to,
					accd_ses_id: accountDetails.ses_id,
					accd_status: accountDetails.accd_status
				});
			} else {
		...
		if (this.terminationFlag) {
			...
			this.feeService.insertFeeAccount(accountJSON).subscribe((result: any) => {
				if (result && result.status === 'ok') {
					...
					this.editChange.emit(this.editFlag);
				} else {
					if (this.permissionFlag) {
						this.editFlag = false;
						} else {
