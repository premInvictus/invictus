...
import { UserAccessMenuService } from 'projects/axiom/src/app/_services/userAccessMenu.service';
...
export class CreateNewUserComponent implements OnInit {
	...
	au_email = new FormControl('', [Validators.required, Validators.email]);
	...
	addEvent(type: string, event: MatDatepickerInputEvent<Date>) {
		...
		const convertedDate = datePipe.transform(
			this.Cu_Form.value.au_dob,
			'yyyy-MM-dd'
		);
		this.Cu_Form.patchValue({
	...
	addUser() {
		...
		if (this.Cu_Form.valid) {
			...
			newUserFormData.append(
				'up_allow_email',
				this.Cu_Form.value.up_allow_email
			);
			newUserFormData.append(
				'up_change_date',
				this.Cu_Form.value.up_change_date
			);
			...
			if (
				this.Cu_Form.value.au_dob < today &&
				this.Cu_Form.value.au_dob !== today &&
				checkDateArray - birthDate > 14
			) {
	...
	setUserForm(value) {
		...
		this.Cu_Form.controls.up_allow_sms.setValue(
			value.privileges[0].up_allow_sms === '1' ? true : false
		);
		this.Cu_Form.controls.up_allow_email.setValue(
			value.privileges[0].up_allow_email === '1' ? true : false
		);
		this.Cu_Form.controls.up_change_date.setValue(
			value.privileges[0].up_change_date === '1' ? true : false
		);
		this.Cu_Form.controls.up_switch_tp.setValue(
			value.privileges[0].up_switch_tp === '1' ? true : false
		);
		this.Cu_Form.controls.up_read_only.setValue(
			value.privileges[0].up_read_only === '1' ? true : false
		);
	}
}
