...
import { NotificationService, BreadCrumbService, HtmlToTextService, UserAccessMenuService, CommonAPIService } from 'projects/axiom/src/app/_services/index';
import { MatPaginator, MatTableDataSource, MatSort } from '@angular/material';
import { SubjectiveElement } from './sreview.modal';
...
@Component({
	selector: 'app-secondary-review-subjective',
	templateUrl: './secondary-review-subjective.component.html',
	styleUrls: ['./secondary-review-subjective.component.css']
})
export class SecondaryReviewSubjectiveComponent implements OnInit {
	...
	openDialog(question) {
		const dialogRef = this.dialog.open(EditSubjectiveDialogSecondary, {
			disableClose: true,
			height: '550px',
			data: {
				question: question,
		...
		this.qbankService.updateQuestion(
			{
				...
				if (result && result.status === 'ok') {
					this.qelementService.publishUnpublishQuestion(value.qus_id, '3', value.qus_unpublish_remark, '', '', 'external')
						.subscribe(
						(result2: any) => {
							if (result2) {
								this.getQuestionsReview();
...
export class EditSubjectiveDialogSecondary implements OnInit, OnDestroy {
	public ind_entry_form1: FormGroup;
	public ind_entry_form2: FormGroup;
	public sub_entry_form3: FormGroup;
	public ind_entry_form3: FormGroup;
	...
	constructor(
		...
		private commonAPIService: CommonAPIService,
		private userAccessMenuService: UserAccessMenuService,
		public dialogRef: MatDialogRef<EditSubjectiveDialogSecondary>,
	...
	ngOnInit() {
		console.log('EditSubjectiveDialogSecondary');
	...
	updateExtQuestion(value, publishstatus = '3') {
		this.qbankService.updateQuestion(
			{
				qus_dl_id: value.qus_dl_id,
				qus_skill_id: value.qus_skill_id,
