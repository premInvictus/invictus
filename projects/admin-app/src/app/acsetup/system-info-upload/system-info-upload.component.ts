...
import { BreadCrumbService, NotificationService } from 'projects/axiom/src/app/_services/index';
...
export class SystemInfoUploadComponent implements OnInit {
	...
	fileChange(event) {
		if (Number(this.SelectForm.value.selectfield) === 2) {
			...
			this.qelementService.userUpload(event.target.files, 3).subscribe(
				(result: any) => {
					if (result && result.status === 'ok') {
						this.notif.showSuccessErrorMessage('File uploaded successfully', 'success');
					} else {
	...
	generateExcel() {
		if (Number(this.SelectForm.value.selectfield) === 1) {
			...
			this.qelementService.userGenerateExcel({role_id: 3}).subscribe(
				(result: any) => {
