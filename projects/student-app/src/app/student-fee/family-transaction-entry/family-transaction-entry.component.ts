...
export class FamilyTransactionEntryComponent implements OnInit {
	...
	ngOnInit() {
		...
		// 	// 	} else {
		// 	// 		this.erpCommonService.getStudentLastRecordPerProcessType().subscribe((result: any) => {
		// 	// 			if (result.status === 'ok') {
	...
	// }
	getSchool() {
		this.erpCommonService.getSchool().subscribe((result: any) => {
			if (result && result.status === 'ok') {
	...
	// 		inv_id: inv_number
	// 	};
	// 	this.erpCommonService.getInvoiceBifurcation(invoiceJSON).subscribe((result: any) => {
	// 		if (result && result.status === 'ok') {
	// 			this.INVOICE_ELEMENT_DATA = [];
	// 			this.dataSource = new MatTableDataSource<InvoiceElement>(this.INVOICE_ELEMENT_DATA);
	// 			this.invoice = {};
	...
	// 		'saveAndPrint': ''
	// 	});
	// 	this.erpCommonService.getStudentInformation({ au_login_id: login_id, au_status: '1' }).subscribe((result: any) => {
	// 		if (result && result.status === 'ok') {
	...
	// }
	getEntryModes() {
		this.erpCommonService.getEntryMode({}).subscribe((result: any) => {
			if (result && result.status === 'ok') {
	...
	getPayModes() {
		this.erpCommonService.getPaymentMode({}).subscribe((result: any) => {
			if (result && result.status === 'ok') {
	...
	getFeePeriods() {
		this.feePeriods = [];
		this.erpCommonService.getFeePeriods({}).subscribe((result: any) => {
			if (result && result.status === 'ok') {
	...
	getBanks() {
		this.banks = [];
		this.erpCommonService.getBanks({}).subscribe((result: any) => {
			if (result && result.status === 'ok') {
	...
	getAllBanks() {
		this.allBanks = [];
		this.erpCommonService.getBanksAll({}).subscribe((result: any) => {
			if (result && result.status === 'ok') {
	...
	getInvoiceTypes() {
		this.invoiceTypes = [];
		this.erpCommonService.getCalculationMethods({}).subscribe((result: any) => {
			if (result && result.status === 'ok') {
	...
	submit() {
		...
		if (validateFlag && this.feeTransactionForm.value.fam_entry_number) {
			this.erpCommonService.insertFeeTransaction(this.feeTransactionForm.value).subscribe((result: any) => {
				if (result && result.status === 'ok') {
