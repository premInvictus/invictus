...
export class ImageViewerComponent implements OnChanges, OnInit, AfterViewInit {
	...
	@Input() showPDFOnlyOption = false;
	@Input() primaryColor = '#161f22';
	...
	ngOnInit() {
		...
		this.allowDownload = this.downloadable === '1' ? true : false
	}
	...
	inicializarImageViewer() {
		this.indexImagemAtual = this.currentImageIndex ? (this.currentImageIndex + 1) : 1;
	...
	injetarIframe(widthIframe: number, heightIframe: number) {
		...
		// iframe.src = 'https://docs.google.com/viewer?url='+`${this.converterPDFBase64ParaBlob()}`+ '&embedded=true' | safe;
		if ($('.image-gallery-2').find('div.iv-image-view')) {
			$('.image-gallery-2').find('div.iv-image-view').removeClass('iv-image-view');
		}
		if (document.getElementById('iframePdf')) {
		...
		this.pdfUrl = this.converterPDFBase64ParaBlob();
		// this.renderer.removeChild(ivImageWrap, iframe);
	...
	isPDF() {
		// console.log('this.getImagemAtual()', this.getImagemAtual(), this.getImagemAtual().endsWith('pdf'));
		if ((this.getImagemAtual() && this.getImagemAtual().endsWith('pdf')) ||
			(this.getImagemAtual() && this.getImagemAtual().endsWith('xlsx')) ||
			(this.getImagemAtual() && this.getImagemAtual().endsWith('xls')) ||
			(this.getImagemAtual() && this.getImagemAtual().endsWith('docx')) ||
			(this.getImagemAtual() && this.getImagemAtual().endsWith('doc'))) {
			return true;
		} else {
			return false;
		}
	...
	isURlImagem() {
		return this.getImagemAtual() && this.getImagemAtual().match(new RegExp(/(https|http|www\.)/g));
	}
