...
import { UserAccessMenuService } from 'projects/axiom/src/app/_services/index';
...
export class SideNavComponent implements OnInit, OnChanges {
	...
	getUserAccessMenu() {
		...
		if (Number(this.currentUser.role_id) === 4) {
			...
			this.userAccessMenuService
				.getUserAccessMenu(param)
				.subscribe((result: any) => {
					if (result.status === 'ok') {
						...
						for (const level0 of this.userAccessMenuArray) {
							const findex = studentMenuArray.findIndex(
								f => Number(f.menu_mod_id) === Number(level0.menu_mod_id)
							);
							if (findex === -1) {
	...
	activateDiv(index) {
		...
		if (menuToggle && menuToggle.menuIndex !== index) {
			...
			showdiv2.style.display =
				menuToggle.display === 'block' ? 'block' : 'none';
		...
		showdiv.style.display =
			showdiv.style.display === 'block' ? 'block' : 'none';
