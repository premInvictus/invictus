...
export class SystemInfoComponent implements OnInit, AfterViewInit {
	...
	toggleStatus(value: any) {
		if (Number(this.configValue) === 1) {
			...
			this.transportService.updateChecklist(value).subscribe((result: any) => {
				if (result) {
	...
	deleteEntry(deletedData, serviceName, next) {
		this.transportService[serviceName](deletedData).subscribe((result: any) => {
			if (result) {
				next(this);
	...
	addEntry(data, serviceName, next) {
		this.transportService[serviceName](data).subscribe((result: any) => {
			if (result) {
