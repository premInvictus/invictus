...
export class SalaryComputationComponent implements OnInit {
			...
			if (result && result.length > 0) {
				...
				for (const item of recordArray) {
					...
					element = {
						...
						emp_pay_scale: item.emp_salary_detail.emp_salary_structure.emp_pay_scale.pc_name,
	...
	save() {
		...
		for (var i=0; i<this.SALARY_COMPUTE_ELEMENT.length;i++) {
			...
			if ('emp_salary_compute_month_id' in this.SALARY_COMPUTE_ELEMENT[i]) {
				edit = true;
			} else {
				edit = false;
			}
